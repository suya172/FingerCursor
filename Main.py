            ...
            for hand_landmark in results.multi_hand_landmarks:
                ...
                for id, landmark in enumerate(hand_landmark.landmark):
                    ...
                    if id == 4:  # 親指の先端
                        a_cv_x, a_cv_y = int(
                            landmark.x*cv_w), int(landmark.y*cv_h)
                        cv2.drawMarker(img, (a_cv_x, a_cv_y), (192, 192, 192),
                                       markerType=cv2.MARKER_CROSS, markerSize=8, thickness=5)
                    if id == 12:  # 中指の先端
                        un_cv_x, un_cv_y = int(
                            landmark.x*cv_w), int(landmark.y*cv_h)
                        cv2.drawMarker(img, (un_cv_x, un_cv_y), (0, 0, 255),
                                       markerType=cv2.MARKER_CROSS, markerSize=8, thickness=5)

                # 描画
                cv2.line(img, (a_cv_x, a_cv_y), (un_cv_x, un_cv_y),
                ...
                draw.text((10, 5), str(distance),  fill=(0, 0, 255),
                          font=font,  stroke_width=1, stroke_fill=(0, 0, 255))
